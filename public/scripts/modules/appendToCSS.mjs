...
appendToCSS = function (
  selector,
  declarations,
  sheetIdName = "dynamic-style-element"
) {
  // See:
  ...
  window.setTimeout(function () {
    ...
    if (selector && declarations) {
      ...
      if (!styleElement) {
        ...
        // remove nonce so an attacker cannot steal it:
      ...
      if (
        declarations.substring(declarations.length - 1, declarations.length) !==
        ";"
      ) {
        declarations += ";";
      }
      ...
      Object.keys(dynamicStyleSheet.cssRules).forEach(function (key) {
        ...
        if (sheetSelector === selector) {
          ...
          try {
            Object.keys(dynamicStyleSheet.cssRules[key].style).forEach(
              function (index) {
                if (isNumeric(index)) {
                  sheetPropertiesArray.push(
                    dynamicStyleSheet.cssRules[key].style[index]
                  );
                } else {
      ...
      if (selectorExists) {
        ...
        if (propertiesMatch) {
          ...
          dynamicStyleSheet.insertRule(
            rule,
            lastCssRuleIndex(dynamicStyleSheet)
          );
        }
      } else {
      ...
      console.log(
        "Error: No selector or declarations provided in call to appendToCSS"
      );
    }
  }, 0);
};
