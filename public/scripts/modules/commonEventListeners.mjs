  ...
  initializeHeaderHeightAndObserver = function () {
    ...
    resizeObserverForHeaderHeight = new ResizeObserver(function (entries) {
      ...
      if (o.headerHeight !== height) {
        ...
        o.appendToCSS(
          ":root",
          "{ --header-height: " + o.headerHeight + "px; }"
        );
      }
    });
...
*/
    ...
    o.appendToCSS(
      ":root",
      "{ --background-color-500ms-ease--border-bottom-color-500ms-ease: background-color 500ms ease, border-bottom-color 500ms ease; }"
    );
    o.appendToCSS(
      ":root",
      "{ --background-color-500ms-ease--border-bottom-color-500ms-ease--color-500ms-ease: background-color 500ms ease, border-bottom-color 500ms ease, color 500ms ease; }"
    );
    o.appendToCSS(
      ":root",
      "{ --header-li-not-first-child-a-transition: border-color 0.5s ease, background 0.5s ease; }"
    );
...
highlightMenuItem = function () {
  ...
  document
    .querySelectorAll("div[data-for^=side-switcher] span")
    .forEach(function (element) {
      element.classList.remove("selected");
    });
